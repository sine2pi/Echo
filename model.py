...
class RecurrentAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        for i in range(0, seq_len, self.chunk_size):
            ...
            print("Shape of query_chunk:", query_chunk.shape)
            print("Shape of key_chunk:", key_chunk.shape)
            print("Shape of value_chunk:", value_chunk.shape)
            
            # Ensure the input chunks are valid
            if query_chunk.size(1) > 0 and key_chunk.size(1) > 0 and value_chunk.size(1) > 0:
                # Call the multihead attention layer
                ...
                raise ValueError("One of the input chunks is empty")
...
class SparseAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        # Debugging information
        print("Shape of query_sparse:", query_sparse.shape)
        print("Shape of key_sparse:", key_sparse.shape)
        print("Shape of value_sparse:", value_sparse.shape)

        # Ensure tensors are non-empty before reshaping
        if query_sparse.shape[1] > 0 and key_sparse.shape[1] > 0 and value_sparse.shape[1] > 0:
            ...
            raise ValueError("Sparse selection resulted in empty tensors")

        attn_output, _ = self.multihead_attn(query_sparse, key_sparse, value_sparse)
        return attn_output
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x, xa=None, mask=None, kv_cache=None):
        ...
        if kv_cache is None or xa is None or 'k' not in kv_cache:
            ...
            if kv_cache is not None:
                kv_cache['k'] = k
                kv_cache['v'] = v
                # print("kv_cache updated:", kv_cache)
        else:
            k = kv_cache['k']
            v = kv_cache['v']
            # print("kv_cache used:", kv_cache)
        ...
        # print("Weighted value wv:", wv)
        # print("Query-Key qk:", qk)
    ...
    def qkv_attention(self, q, k, v, mask=None):
        ...
        # print("Detached qk:", qk)

        return out, qk

        # self.local_attn = SparseAttention(n_state, n_head, max_rel_dist, base, sparsity_factor)
