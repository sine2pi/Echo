...
class HybridAttention(nn.Module):
    def __init__(self, n_state, n_head, n_layer, window_size, sparsity_factor=0.333, 
...
class AudioEncoder(nn.Module):
    def __init__(self, n_mels, n_ctx, n_state, n_head, n_layer, window_size, max_rel_dist, checkpointing, cross_attention=True, base=10000):
        super().__init__()
...
class Echo(PreTrainedModel):
    ...
    def print_update(self, x):
        ...
        if self.adjust_counter % 100 == 0:
            print(f"Training: {self.adjust_counter}: Loss: {self.best_loss}  Base: {self.base}, Window size: {self.window_size}")
        return x
    ...
    def on_evaluate(self, args, state, control, model, metrics=None, **kwargs):
        ...
        if self.predictions is not None and self.label_ids is not None:
            ...
            if state.global_step % self.log_every_n_steps == 0:
                ...
                print(f"Evaluation: - Step {state.global_step} - Loss: {eval_loss:.4f} - Adjusted Base: {adjusted_base:.4f} - Adjusted Window: {adjusted_window_size:.4f}")
...
def prepare_dataset(batch):
    ...
    for audio_item in audio:
        processed_features = feature_extractor(audio_item["array"], sampling_rate=audio_item["sampling_rate"]).input_features[0]
        input_features.append(processed_features)
    batch["input_features"] = input_features
    labels = []
    for text_item in batch["text"]: 
        processed_labels = tokenizer(text_item).input_ids
        labels.append(processed_labels)
    batch["labels"] = labels
    return batch


def create_dataset_chunk(data_chunk):
    return Dataset.from_dict({k: [d[k] for d in data_chunk] for k in data_chunk[0].keys()})

dataset = load_dataset("fixie-ai/librispeech_asr", "clean", split="train.100", streaming=False, trust_remote_code=True, cache_dir="d:/hf")
dataset = dataset.map(prepare_dataset, batched=True, batch_size=1000).select_columns(["input_features", "labels"])

kf = KFold(n_splits=5)
accuracies = []
eval_size = 50  # eval_size = int(len(val_index) * 0.2
...
for fold, (train_index, val_index) in enumerate(kf.split(dataset)):  
    ...
    accuracies.append(eval_results["eval_wer"])

average_wer = sum(accuracies) / len(accuracies)
